...
class OxigenacionScreen extends StatefulWidget {
  const OxigenacionScreen({super.key});

  @override
  State<OxigenacionScreen> createState() => _OxigenacionScreenState();
}

class _OxigenacionScreenState extends State<OxigenacionScreen> with SingleTickerProviderStateMixin {
  final _formKey = GlobalKey<FormState>();
  DateTime? _selectedDate;
  TimeOfDay? _selectedTime;
  DateTime? _searchDate;
  final _oxigenacionController = TextEditingController();
  ...
  Future<void> _fetchRegistros({String? fecha}) async {
    ...
    try {
      final uri = Uri.parse('https://ec1bff1533be.ngrok-free.app/api/oxigenaciones')
      ...
      print('Respuesta del servidor (GET /api/oxigenaciones): ${response.statusCode} - ${response.body}');
  ...
  Future<void> _submitRegistro() async {
    ...
    final int? valor = int.tryParse(_oxigenacionController.text);
    if (valor == null || valor < 0 || valor > 100) {
      setState(() {
        _mensaje = 'Valor de oxigenación inválido (0 - 100)';
      });
      return;
    }
    setState(() {
  ...
  Future<void> _updateRegistro(int id, DateTime fecha, TimeOfDay hora, int valor) async {
    setState(() {
      _isLoading = true;
      _mensaje = null;
    });
    ...
    try {
      final response = await http.put(
        Uri.parse('https://ec1bff1533be.ngrok-free.app/api/oxigenaciones/$id'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
          'ngrok-skip-browser-warning': 'true',
      ...
      print('Respuesta del servidor (PUT /api/oxigenaciones/$id): ${response.statusCode} - ${response.body}');
  ...
  Future<void> _deleteRegistro(int id, String fecha, String hora) async {
    ...
    try {
      ...
      print('Respuesta del servidor (DELETE /api/oxigenaciones/$id): ${response.statusCode} - ${response.body}');
  ...
  void _showEditDialog(dynamic registro) {
    ...
    final editOxigenacionController = TextEditingController(text: registro['valor'].toString());
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 12),
                TextFormField(
                  controller: editOxigenacionController,
                  decoration: InputDecoration(
                    labelText: 'Oxigenación (%)',
                  ...
                  validator: (value) {
                    if (value == null || value.isEmpty) {
                      return 'Introduce el nivel de oxigenación';
                    }
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                final int? valor = int.tryParse(editOxigenacionController.text);
                if (valor == null || valor < 0 || valor > 100) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Valor de oxigenación inválido')),
                  );
                  return;
                }
                Navigator.pop(context);
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Text(
            'Confirma la eliminación del registro de oxigenación del ${registro['fecha']} a las ${registro['hora'].substring(0, 5)} (ID: ${registro['id']})',
            style: const TextStyle(fontFamily: 'Roboto', color: Colors.black87),
          ),
          actions: [
            TextButton(
...
backgroundColor: Colors.red[700],
                          ...
                          Form(
                            ...
                            child: Column(
                              children: [
                                ...
                                SizedBox(height: size.height * 0.01),
                                TextFormField(
                                  controller: _oxigenacionController,
                                  decoration: InputDecoration(
                                    labelText: 'Oxigenación (%)',
                                    labelStyle: const TextStyle(color: Colors.black87, fontFamily: 'Roboto'),
                                    prefixIcon: Icon(Icons.air, color: Colors.blue[700], size: size.width * 0.04),
                                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(10)),
                                    filled: true,
                                    fillColor: Colors.grey[100],
                                  ),
                            ...
                            child: Column(
                              ...
                              children: [
                                _buildRangeRow(
                                  ...
                                  range: '- 90%',
                                  description: 'Oxigenación muy baja',
                                ),
                                SizedBox(height: size.height * 0.01),
                                _buildRangeRow(
                                  context: context,
                                ...
                                _buildRangeRow(
                                  ...
                                  range: '95 - 100%',
                                  description: 'Oxigenación normal',
                                ),
                              ],
                            ),
                          ),
                        ...
                        children: [
                          ...
                          _isLoading
                              ...
                              : _registros.isEmpty
                                  ? Center(
                                      child: Text(
                                        'No hay registros de oxigenación.',
                                        style: TextStyle(
                                          fontSize: size.width * 0.04,
                                          fontFamily: 'Roboto',
                                  ...
                                  : RefreshIndicator(
                                      ...
                                      child: ListView.builder(
                                        ...
                                        itemBuilder: (context, index) {
                                          ...
                                          final valor = (registro['valor'] as num).toInt();
                                          final isVeryLow = valor < 90;
                                          final isLow = valor >= 90 && valor < 95;
                                                  ...
                                                  children: [
                                                    ...
                                                    if (isVeryLow || isLow)
                                                      Padding(
                                                        ...
                                                        child: Icon(
                                                          isVeryLow ? Icons.warning : Icons.info,
                                                          color: isVeryLow ? Colors.red[700] : Colors.yellow[700],
  ...
  @override
  void dispose() {
    _oxigenacionController.dispose();
...
class TVGraphScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final maxY = registros.isNotEmpty
        ...
        : 100.0;
    final yInterval = maxY > 100 ? 20.0 : 10.0;
    ...
    for (var i = 0; i < registros.length; i++) {
      ...
      if (valor < 90) {
        lowSpots.add(spot);
        print('Valor $valor en índice $i asignado a lowSpots (rojo)');
      } else if (valor <= 94) {
        midSpots.add(spot);
        print('Valor $valor en índice $i asignado a midSpots (amarillo)');
      } else {
        highSpots.add(spot);
        print('Valor $valor en índice $i asignado a highSpots (verde)');
      }
    }
    final Color lowColor = Colors.red[700] ?? const Color(0xFFD32F2F); 
    final Color midColor = Colors.yellow[700] ?? const Color(0xFFFFCA28); 
    final Color highColor = Colors.green[700] ?? const Color(0xFF388E3C); 
